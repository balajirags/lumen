...
4. **Wire in the pretty logging (`codedoc.log`) — do not skip this.** The agent-stage
   module must call `start_agent_boxes(agent_names=..., workflow_phases=...)`,
   `update_agent_box`, `print_researcher_done`, `print_tool_usage_table`,
   `update_workflow_phase`, `print_synthesizer_done`, and `stop_agent_boxes` with its own
   role/phase names — see the "Reusable building blocks" table and the `run_agent`
   skeleton in the document. `run_loop` by itself does not produce `lumen run`-style
   output; without this wiring the console falls back to plain dim-text lines with no
   live boxes, no "researcher done" checkmarks, and no tool-usage table. This has
   regressed once already and had to be retrofitted — treat it as a required step, not a
   nice-to-have.
5. Follow the document's "What NOT to touch" list strictly — never modify
...
6. Run the document's "Verify" checklist before considering the work done, including a
   smoke test against a small synthetic repo (not a large real one — keep token/time cost
   low) that confirms the fan-out roles actually ran concurrently, the fan-in step produced
   the final artifact, and the pretty-logging output (checkmarks, tool-usage table,
   synthesizer-done line) actually appeared. Clean up smoke-test output before finishing.
7. If, while implementing, you find the document itself is unclear, missing a step, or the
