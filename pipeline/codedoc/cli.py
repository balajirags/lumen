...
from codedoc.pipelines.security_audit import run_pipeline as run_security_audit_pipeline
...
def common_pipeline_options(f):
    """Shared indexer/agent options for commands that run preflight + indexer + an agent stage.

    Any new pipeline command (index a repo, run its own agent stage against the graph)
    reuses these options instead of re-declaring them.
    """
    options = (
        ...
        click.option("--allow-xlarge", is_flag=True, default=False, help="Allow the pipeline to continue even when preflight classifies the repo as xlarge"),
...
def security_audit(
    ...
    """Run the security-audit pipeline: index -> fan-out reviewers -> risk synthesis.

    REPO_PATH is the local path to the source code repository. Reuses the same
    preflight+indexer stages as `lumen run`, but runs a security-focused fan-out/fan-in
    agent stage instead of the docs pipeline's Analyst+Architect supervisor.
    """
    cfg = load_config({
        "output_dir": output_dir,
        "model": model,
    ...
    state = run_security_audit_pipeline(
        repo_path=repo_path,
        repo_name=repo_name,
        output_dir=cfg.output_dir,
        model=cfg.model,
