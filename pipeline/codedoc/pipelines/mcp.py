"""MCP-focused Lumen pipeline orchestration."""
...
from codedoc.mcp_server import format_mcp_command
...
def run_mcp_pipeline(
    ...
    """Execute the preflight + indexer path and prepare MCP serving metadata."""
    ...
    log_pipeline_start(state, repo_path=str(repo), run_dir=run_dir, label="MCP pipeline")
    ...
    try:
        ...
        state.mcp_command = format_mcp_command(state.kuzu_path, state.repo_path)
        state.status = "done"
        pipeline_elapsed = time.time() - pipeline_t0
        state.log("pipeline", f"MCP pipeline completed successfully in {pipeline_elapsed:.1f}s")
    except Exception as exc:
        state.status = "failed"
        state.error = str(exc)
        state.log("pipeline", f"MCP pipeline failed: {exc}")
    finally:
